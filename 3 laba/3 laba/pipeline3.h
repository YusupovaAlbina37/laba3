...
public:
	...
	{
		m_camera.Pos = Pos; //позици€ камеры
...
private:
	...
	vec3 m_worldPos; //позици€ в мире
	vec3 m_rotateInfo; //вращение

	//перспективна€ проекци€
	struct {
		float FOV; //поле зрени€
		float Width; //ширина
		float Height; //высота
		float zNear; //ближн€€ плоскость
		float zFar; //дальн€€ плоскость
	} m_persProj;

	//параметры камеры
	struct {
		vec3 Pos; //позици€ камеры
